...
- WASI Preview 3 is tracked at `0.3.0-rc-2026-03-15`. `WasiPreview3` exposes
  release-candidate metadata, synchronous host imports for `wasi:cli/environment`
  and `exit`, `wasi:clocks/system-clock`, `monotonic-clock`, and timezone,
  ...
  inbound `handler.handle` and `wasi:http/client.send` backed by Ktor when
  networking is explicitly enabled. `wasi:sockets`
  release-candidate imports expose DNS lookup and TCP/UDP socket resources for
  create, bind, connect, listen/accept handles, address accessors, and socket
  options.
- Full Preview 3 async runtime semantics are not complete yet. `future` and
  `stream` currently lower/lift as typed `WitFuture<T>` and `WitStream<T>`
  canonical handles with immediate placeholder completions for stdio,
  filesystem stream handoff, HTTP body/trailer handoff, and lazy host-side TCP
  receive streams; event-loop integration, real cross-task stream transport,
  HTTP trailer streaming beyond host-resolved futures, guest-side canonical
  stream read/write operations for Preview 3 TCP send/receive/listen, and fully
  asynchronous UDP receive scheduling still need dedicated host implementations
  as the RC surface settles.
...
- `WasiPreview`, which marks Preview 2 as stable Component Model metadata and
  Preview 3 as release-candidate Component Model metadata.
...
`task.return`. For async callback components, package with
...
WASI 0.3/WASIp3 is exposed as release-candidate Component Model metadata. WIT
